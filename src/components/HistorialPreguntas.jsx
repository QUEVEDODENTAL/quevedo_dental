...
function HistorialClinicoForm() {
  ...
  const [colorationTongueSelected, setColorationTongueSelected] =
    useState(null);
  const [tongueUlcerations, setTongueUlcerationsss] =
    useState("sin-ulceraciones");
  ...
  const optionsColorationTongue = ["Blanca", "Gris", "Rosa", "Roja"];
  ...
  const renderDiseasesOptions = () => {
    return enfermedadesList.map((enfermedad, index) => (
      <div key={index} className="flex items-center justify-between mb-2">
        <label htmlFor={enfermedad.name} className="mr-2">
          {enfermedad.label}
        </label>
        <ReactSwitch
          id={enfermedad.name}
  ...
  const renderColorationTongueOptions = () => {
    return optionsColorationTongue.map((coloracion, index) => (
      <div
        key={index}
        className="coloracion-lengua-option flex flex-col items-center"
      >
        <input
          type="checkbox"
        ...
        />
        <label
          htmlFor={`coloracionLengua-${index}`}
          className={`flex flex-col items-center cursor-pointer ${
            coloracion === colorationTongueSelected
  ...
  const Tooth = ({ number, src, isSelected, onClick }) => (
    <div
      className={`flex flex-col items-center shadow-md cursor-pointer p-2 rounded hover:text-primary-seccion hover:bg-secondary-button ${
        isSelected
          ? " bg-secondary-button text-primary-white"
          : " bg-primary-seccion"
      }`}
      onClick={onClick}
    >
      ...
      {isSelected && (
        <Image
          ...
          height={90}
          src={src}
          alt={`Diente ${number} seleccionado`}
        />
      )}
    </div>
  );
    ...
    <div className="mb-4">
      ...
      {currentPage === 1 && (
        <form onSubmit={handleSubmit} className="max-w-md">
          ...
          <label htmlFor="name" className="border-black">
            Nombre:
          </label>
          <input
            type="text"
          ...
          <div className="form-control mb-3">
            <label htmlFor="address" className="border-black">
              Domicilio:
            </label>
            <input
              type="text"
          ...
          <div className="form-control mb-3">
            <label htmlFor="phone" className="border-black">
              Teléfono:
            </label>
            <input
              type="text"
          ...
          <div className="form-control mb-3">
            <label htmlFor="ocupation" className="">
              Ocupación:
            </label>
            <select
              id="ocupation"
              value={ocupationSelected}
          ...
          <div className="form-control mb-3">
            <label htmlFor="fecha_nacimiento" className="border-black">
              Fecha de Nacimiento:
            </label>
            <input
              type="date"
          ...
          <div className="form-control mb-3">
            <label htmlFor="city" className="border-black">
              Ciudad:
            </label>
            <input
              type="text"
          ...
          <fieldset>
            <legend className="text-lg font-semibold border-black mb-3">
              Padecimiento Actual
            </legend>
            <div className="form-control">
              <label htmlFor="reasonConsultation" className="border-black">
                Motivo de consulta:
              </label>
              <textarea
                id="Consultation"
          ...
          <fieldset>
            <div className="mb-4">
              ...
              <button
                type="button"
                onClick={nextPage}
                className=" p-2 bg-secondary-card rounded-lg text-primary-white hover:bg-secondary-dash transition-colors duration-300 w-full"
              >
                Siguiente
              </button>
            </div>
      ...
      {currentPage === 2 && (
        <div className="flex flex-col items-center p-4">
          <div className="px-4 py-2 rounded mb-4">Odontograma</div>
          ...
          <div className="flex justify-between w-full mt-4">
            <button
              type="button"
              onClick={prevPage}
              className="w-1/2 p-2 bg-secondary-card rounded-lg text-primary-white hover:bg-secondary-dash transition-colors duration-300 mr-2"
            >
              Anterior
            </button>
            <button
              type="button"
              onClick={nextPage}
              className="w-1/2 p-2 bg-secondary-card rounded-lg text-primary-white hover:bg-secondary-dash transition-colors duration-300 ml-2"
            >
              Siguiente
            </button>
          </div>
          ...
          <fieldset>
            ...
            <div className="form-control">
              ...
              <div className="color-options">
                {["#FFCCCC", "#FF0000", "#8B0000", "#FFFFFF", "#4B0082"].map(
                  (color, index) => (
                    <div
                      key={index}
                      className="color-option"
                      onClick={() => handleGumColorationChange(color)}
                      style={{ backgroundColor: color }}
                    >
                      <div className="color-box"></div>
                      <div className="color-name">
                        {traducirColorEncias(color)}
                      </div>{" "}
                      {/* Muestra el nombre del color en lugar del código hexadecimal */}
                    </div>
                  )
                )}
              </div>
              ...
              >
                ...
                {gumColoration && (
                  <div
                    className="color-overlay rounded-3xl"
                    style={{
                      position: "absolute",
                      top: 0,
                      left: 0,
            ...
            <div className="form-control">
              <label htmlFor="observationsTongue" className="">
                Observaciones en Lengua:
              </label>
              <textarea
                ...
                placeholder="Escriba aquí cualquier observación sobre la lengua..."
              />
            </div>
            ...
            <div className="form-control">
              <label htmlFor="palateColoring" className="">
                Coloración de Paladar Duro:
              </label>
              <br />
              <select
                id="palateColoring"
                value={palateColoring}
                onChange={(e) => setPalateColoring(e.target.value)}
                className="border rounded-md p-2 w-full"
              >
                <option value="">Seleccionar coloración de Paladar</option>
                {optionsPalateColoring.map((coloracion, index) => (
                  <option key={index} value={coloracion}>
                    {coloracion}
                  </option>
                ))}
              </select>
            ...
            <div className="form-control">
              <label htmlFor="palateInjuries" className="">
                Lesiones en el paladar:
              </label>
              <select
                id="palateInjuries"
                value={palateInjuries}
                onChange={(e) => setPalateInjuries(e.target.value)}
                className="border rounded-md p-2 w-full"
              >
            ...
            <div className="form-control">
              <label htmlFor="observationsPalate" className="">
                Observaciones en el paladar:
              </label>
              <textarea
                ...
                placeholder="Escriba aquí cualquier observación sobre el paladar..."
              />
            </div>
            <div className="flex justify-between mt-4">
              <button
                type="button"
                onClick={prevPage}
                className=" p-2 bg-secondary-card rounded-lg text-primary-white hover:bg-secondary-dash transition-colors duration-300"
              >
                Anterior
              </button>
        ...
        <div>
          ...
          <PDFViewer style={{ width: "100%", height: "600px" }}>
            <HistorialClinicoPDF
              ...
              toothSelectedUpdated={{ toothSelected }}
              toothImage={toothImage}
            />
          </PDFViewer>
          ...
          <div className="items-center justify-center text-base py-3">
            ...
            >
              {({ loading }) => (
                <button
                  disabled={loading}
                  className=" w-full px-24 py-2 min-w-[50%] md:min-w-[200px] bg-secondary-button text-white rounded hover:bg-secondary-dash hover:text-primary-white transition-colors duration-300"
                >
